...
object Main extends App {
    ...
    val datacsv = spark.read.options(Map("inferSchema" -> "true", "delimiter" -> ",", "header" -> "true"))
    //  .schema("VendorID integer, tpep_pickup_datetime timestamp, tpep_dropoff_datetime timestamp, passenger_count integer, trip_distance double, RatecodeID integer, store_and_fwd_flag string, PULocationID integer, DOLocationID integer, payment_type integer, fare_amount double, extra double, mta_tax double, tip_amount double, tolls_amount double, improvement_surcharge double, total_amount double, congestion_surcharge double")
      .csv("src/main/data/yellow_tripdata_2020-01.csv")
    //datacsv.printSchema()


    // группируем сторно строки
    ...
    // создаем столбец даты из столбца даты и времени
    // разбиваем кол-во пассажиров на группы
...
//    datacsv1.printSchema()
//    datacsv1.show(50)

    // формируем необходимый набор данных
    ...
    // сохраняем в формат parquet разделяя по дате
    df.write
      .partitionBy("date")
      .parquet("src/main/data/tripdata.parquet")

    //println(df.count())

    // ------create table

    val parquet_data = spark.read.parquet("src/main/data/tripdata.parquet")
    ...
    // кол-во и макс(стоимость поездки), группируя по дате и кол-ву пассажиров
    val group_data1 = tab_data.groupBy("date", "passanger")
      .agg(count("*").as("p_count"),
        max("total_amount").as("max_total_amount"))

    // кол-во, группируя по дате
    val group_data2 = tab_data.groupBy("date").count()

    // мин(стоимость поездки), группируя по дате и кол-ву пассажиров, где total_amount > 0
    val group_data3 = (tab_data.select("date", "passanger", "total_amount")
      .where(col("total_amount") > 0))
      .groupBy("date", "passanger")
      .min("total_amount")

    // объединяем группы и рассчитываем процент
    ...
    // формируем таблицу с необходимыми названиями столбцов
    ...
    val parquetDFall = spark.read.parquet("src/main/data/tripdata.parquet")
      .where("date >= '2020-01-01' and date <= '2020-01-31'  ")
    parquetDFall.createOrReplaceTempView("ParquetTable2")
    val graf_data = spark.sql("select * from ParquetTable2")

    //println(graf_data.count())

    // выбираем уникальные значения, исключая отдельные выбросы
    ...
    //  val dfm3 = data_grafic.groupBy("trip_distance")
    //                        .agg(
    //                              mean("tip_amount").as("tmean_tip_amount"),
    //                              min("tip_amount").as("tmin_tip_amount"),
    //                              max("tip_amount").as("tmax_tip_amount")
    //                            )

    //  val dfm4 = data_grafic.groupBy("passenger_count")
    //                        .agg(
    //                              mean("tip_amount").as("pmean_tip_amount"),
    //                              min("tip_amount").as("pmin_tip_amount"),
    //                              max("tip_amount").as("pmax_tip_amount")
    //                            )


    //println(data_grafic.count())
